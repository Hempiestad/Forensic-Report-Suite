# Build Script for Komainu Labs Forensic Report Suite
# Version: 1.0
# Date: February 3, 2026
# Description: Automated build script with pre-checks and post-build verification
...
Write-Host "║  Version 1.2 - Alpha Build                                     ║" -ForegroundColor Cyan
Write-Host "╚════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
if (-not $Client -and -not $Server -and -not $Both) {
    ...
    Write-Info "Example: .\build.ps1 -Client"
    exit 1
}
...
Write-Info "`n═══ Phase 4: Post-Build Verification ═══"
Write-Host ""
...
if ($Client) {
    $clientExe = Join-Path $distPath "ForensicReportWriter\ForensicReportWriter.exe"
...
if ($Server) {
    $serverExe = Join-Path $distPath "ForensicCaseServer\ForensicCaseServer.exe"
...
$docFiles = @("README.md", "MAIN_USER_GUIDE.md", "NOTES_USER_GUIDE.md", "REPORTS_USER_GUIDE.md", "INSTALLATION_GUIDE.md")
...
if ($buildSuccess) {
    ...
    if ($Client) {
        Write-Info "  Client: dist\ForensicReportWriter\"
    }
    if ($Server) {
        Write-Info "  Server: dist\ForensicCaseServer\"
    }
    
    Write-Host ""
    Write-Info "Next Steps:"
    Write-Host "  1. Test the executable(s) thoroughly" -ForegroundColor White
    Write-Host "  2. Check for any missing DLLs or dependencies" -ForegroundColor White
